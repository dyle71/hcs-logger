...
2. Launch a builder
```bash
$ docker run -it -d --rm --name hcs-logger_debian-buster hcs-logger:debian-buster
```
...
```bash
$ docker exec -it hcs-logger_debian-buster git clone https://gitlab.com/headcode.space/logger.git . 
...
$ docker exec -it hcs-logger_debian-buster /bin/bash
...
```bash
$ docker stop hcs-logger_debian-buster 
```
