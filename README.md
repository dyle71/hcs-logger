...
3. Provide software with super minimal interfaces. I try to really provide absolute minimum without 
...
4. Provide libraries and binaries with little to no runtime dependencies. Linkage though might
   require additional libraries (sometimes from `headcode.space` sometimes else). Yet, whenever
   possible I strive to go for the static libraries, because a) the resulting binaries have
   little to no runtime depenency and can be installed directly with a mere `copy` and
